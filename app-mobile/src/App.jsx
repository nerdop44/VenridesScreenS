...
const ServerConfigModal = ({ isOpen, onClose, currentUrl, onSave }) => {
    ...
    return (
        <div className="modal-overlay" style={{ zIndex: 10000 }}>
            <div className="modal-content glass-card" style={{ maxWidth: '400px', border: '1px solid rgba(255,255,255,0.1)' }}>
                <h3 style={{ marginBottom: '1rem', color: 'var(--primary-color)' }}>Configuración de Servidor</h3>
                <p style={{ fontSize: '0.85rem', opacity: 0.7, marginBottom: '1rem' }}>
                    Ingrese la URL completa del backend (API). <br />
                    Ej: <code>http://192.168.1.200:8000</code>
                </p>
                <input
                    value={url}
                    onChange={e => setUrl(e.target.value)}
                    placeholder="http://192.168.1.182:8000"
                    style={{ marginBottom: '1rem' }}
                />
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <button onClick={onClose} className="btn" style={{ flex: 1 }}>Cancelar</button>
                    <button onClick={() => onSave(url)} className="btn btn-primary" style={{ flex: 1 }}>Guardar y Reiniciar</button>
                </div>
            </div>
            <div style={{ marginTop: '1rem', borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '1rem' }}>
                <button onClick={async () => {
                    ...
                    try {
                        ...
                        const res = await fetch(`${url.replace(/\/$/, '')}/diag/status`, { signal: controller.signal });
                        clearTimeout(timeoutId);
                        if (res.ok) {
                            status.innerText = "✅ Conexión Exitosa (Servidor OK)";
                            status.style.color = "var(--success)";
                        } else {
                            status.innerText = `⚠️ Error HTTP: ${res.status}`;
                            status.style.color = "var(--warning)";
                        }
                    } catch (e) {
                        status.innerText = `❌ Fallo: ${e.message}`;
                ...
                }} id="test-btn" className="btn btn-secondary" style={{ width: '100%', marginBottom: '0.5rem' }}>
                    Probar Conexión
                </button>
                <p id="test-status" style={{ fontSize: '0.8rem', textAlign: 'center', marginTop: '0.5rem', fontWeight: 'bold' }}></p>
            </div>
        </div>
    );
};
...
function App() {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                else if (data.user.role === 'admin_empresa') {
                    targetView = 'client';
                    setClientTab('videos');
                }
    ...
    useEffect(() => {
        ...
        fetchUnread();
        const interval = setInterval(fetchUnread, 60000); // Pulse every minute
        ...
        return (
            <div className="login-screen">
                <div style={{ position: 'absolute', top: '1.5rem', right: '1.5rem', zIndex: 10 }}>
                    <button onClick={() => setShowConfigModal(true)} style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', color: 'white', cursor: 'pointer', padding: '0.5rem', borderRadius: '50%' }} title="Configurar Servidor">
                        <Settings size={20} />
                    </button>
                </div>

                {showConfigModal && <ServerConfigModal
    ...
    // MOBILE VIEW INTERCEPTION
    // MOBILE VIEW INTERCEPTION (Clients Only - Admins stay on Responsive Desktop)
    if (isMobile && token) {
        const userObj = safeParse(localStorage.getItem('user'));
        const userRole = localStorage.getItem('user_role');
        // Only divert to MobileApp if user is a client (user_basic only)
        // Admin Empresa now gets full responsive dashboard for feature parity
        if (['user_basic'].includes(userRole)) {
            return <MobileApp token={token} userObj={userObj} onLogout={handleLogout} />;
        }
    }
        ...
        return (
            <div className="dashboard-container">
                ...
                <header className="dash-header">
                    <h1 style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap', fontSize: '1.2rem' }}>
                        <img src="/venrides_logo.png" alt="Admin" className="app-logo" style={{ maxHeight: '50px', width: 'auto' }} />
                ...
                <div className="admin-tabs" style={{ background: 'var(--card-bg)', padding: '0.5rem', borderRadius: '12px', marginBottom: '1.5rem', display: 'flex', gap: '0.5rem', border: '1px solid var(--border-color)' }}>
                        ...
                        <div className="table-responsive">
                            <table className="admin-table">
                                ...
                                <tbody>
                                    {companies.map(c => (
                                        <tr key={c.id}>
                                            <td data-label="Empresa" style={{ fontWeight: '600' }}>{c.name}</td>
                                            <td data-label="Plan" style={{ textTransform: 'uppercase', fontSize: '0.8rem' }}>{c.plan}</td>
                                            <td data-label="Screens"><span className="badge-screens">{c.total_screens} / {c.max_screens} TV</span></td>
                                            <td data-label="Estado">
                                                <span className={`badge-status ${c.is_active ? 'active' : 'inactive'}`}>
                                                    {c.is_active ? '✓ Activo' : '✗ Suspendido'}
                                                </span>
                                            </td>
                                            <td data-label="Vencimiento" style={{ fontSize: '0.8rem' }}>{c.valid_until ? new Date(c.valid_until).toLocaleDateString() : 'N/A'}</td>
                                            <td data-label="Acciones" style={{ display: 'flex', justifyContent: 'flex-end' }}>
                                ...
                                <tbody>
                                    {users.map(u => (
                                        <tr key={u.id}>
                                            ...
                                            <td data-label="Empresa">{companies.find(c => c.id === u.company_id)?.name || 'N/A'}</td>
                                            <td data-label="Rol">
                                            ...
                                            <tbody>
                                                {devs.map(d => (
                                                    <tr key={d.id}>
                                                        ...
                                                        <td data-label="Empresa" style={{ fontSize: '0.85rem', opacity: 0.7 }}>{compName}</td>
                                                        <td data-label="UUID" style={{ fontSize: '0.7rem', opacity: 0.5, fontFamily: 'monospace' }}>{d.uuid}</td>
                                                        <td data-label="Estado">
                        ...
                        <div className="table-responsive">
                            <table className="admin-table">
                                ...
                                <tbody>
                                    {payments.map(p => (
                                        <tr key={p.id}>
                                            <td data-label="Fecha">{new Date(p.payment_date).toLocaleDateString()}</td>
                                            <td data-label="Empresa" style={{ fontWeight: '500' }}>{companies.find(c => c.id === p.company_id)?.name || p.company_id}</td>
                                            <td data-label="Monto" style={{ color: '#10b981', fontWeight: 'bold' }}>{p.currency} {p.amount}</td>
                                            <td data-label="Método" style={{ fontSize: '0.8rem', opacity: 0.7 }}>{p.payment_method}</td>
                                            <td data-label="Acciones">
                ...
                {adminTab === 'stats' && (
                    <div className="glass-card">
                        ...
                        {stats && (
                            <div className="stats-grid" style={{ marginTop: '1rem', flexWrap: 'wrap' }}>
            ...
            <table className="admin-table">
                ...
                <tbody>
                    {users.filter(u => u.role === 'operador_empresa').map(u => (
                        <tr key={u.id}>
                            <td data-label="Usuario (Email)">{u.username}</td>
                            <td data-label="Rol">Operador</td>
                ...
                <tbody>
                    {items.map(i => (
                        <tr key={i.id}>
                            ...
                            <td data-label="Precio" style={{ fontWeight: 'bold' }}>${i.price}</td>
                            <td data-label="Eliminar"><button onClick={async () => { await fetch(`${API_BASE}/admin/menus/${i.id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` } }); setItems(items.filter(x => x.id !== i.id)); }} className="action-btn suspend"><Trash2 size={12} /></button></td>
                        </tr>
                    ))}
                </tbody>
            </table>
                ...
                <div className="table-responsive">
                    <table className="admin-table">
                        ...
                        <tbody>
                            {tickets.map(t => (
                                <tr key={t.id}>
                                    <td data-label="Asunto" style={{ fontWeight: '600' }}>{t.subject}</td>
                                    ...
                                    <td data-label="Actualizado" style={{ fontSize: '0.8rem' }}>{new Date(t.updated_at).toLocaleString()}</td>
                                    <td data-label="Acción"><button className="action-btn view" onClick={() => fetchTicketDetails(t.id)}><Eye size={16} /></button></td>
                                </tr>
                            ))}
